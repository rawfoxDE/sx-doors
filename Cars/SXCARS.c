...
void Door(void); // das Game
void Race(void); // raceroutine
...
void drei(void);
void killnum(void);
void Selection(void);// die Carselection Page
...
extern void raceend(int winner, int second, int third, int carflag);
...
keypress:                         
        ...
        PS("\033[2J");
	   XIMFunction(8,0,"doors:sxcars/car_mainmenu.ans");
        ...
        PS("[13;33H");
        sprintf(buf,"%ld",Player.wins);
        PS(buf);

        //PS("[14;33H");
        //sprintf(buf,"%ld",Player.cash);
        //PS(buf);
        ...
        switch(themsg.String[0])
        {
                ...
                {
                        PS("\033[2J");
                        XIMFunction(8,0,"doors:sxcars/car_doc.ans");
                        XIMFunction(6,0,"\n\rReturn to go back...: ");
                        goto keypress;
                }
                case 'c':
...
void Race(void)
{
        ...
        char buf[256], target[2];                                // y      = counter for mainloop        
        int winner=0, second=0, third=0;                         // z      = zufallszahl -> welches auto bewegen    

        PS("\033[2J");                                            
        XIMFunction(8,0,"doors:sxcars/car_race.ans");    //   gfx
	...
	XIMFunction(6,0,"\n\rpress to R A C E ! ...(ret/q) \n\r");
        ...
        while(won<3)                          //...mer fahren bis zum Sieg !
        {
                ...
                if(z==1 && camov<=29)     // AUTO 1                        
                {
                         camov++;
                         cara(camov);    
                         if(camov==30) 
                         {
                                won++; // camov schon 30 ?..gewonnwn ?
                                strcpy(target,"1");
                         }       
                }         
                if(z==2 && cbmov<=29)                                                                         // AUTO 2  
                {
                         cbmov++;
                         carb(cbmov);
                         if(cbmov==30) 
        ...
        raceend(winner,second,third,carflag);
        
        //sprintf(buf,"[24;0HWINNER: %s \n\rSECOND: %s \n\rTHIRD : %s\n\r", winner, second, third);
        //PS(buf);
          
        if(carflag==winner) 
        {       PS("\n\n\n\r YYOOUU WWIINN!!");
                winflag++;  // WINs counter !
        }
        else 
        {       PS("\n\n\n\r...not a winner ..:( ");
        }
...
void Selection(void)
{
       ...
       // int carflag;   
        ...
        XIMFunction(8,0,"doors:sxcars/car_selection.ans");    //   gfx
...
Auswahl:        
	...
	XIMFunction(5,1,"\n\r\n\r[0;33;44mSelect your Car[0m : ");
       ...
       XIMFunction(6,1,"ok, press M for Menu, get to (r)ace or select (a)gain...");
       ...
       switch(themsg.String[0])
       {
        ...
        case 'R':
        case 'r': 
           Race();   // zum rennen mit dem gewaehlten Auto
           break;
        case 'A':
        case 'a': 
           goto Auswahl;   // erneut auswaehlen
                
        case 'M': 
        case 'm':  
           break; // zurueck zum Mainmenue        
...
{       
        ...
        fhpl = Open("doors:sxcars/car_log", MODE_OLDFILE);
        if(fhpl==NULL) 
        {
                fhpl = Open("doors:sxcars/car_log", MODE_NEWFILE);
        ...
        } else if(fhpl)
        {
                ...
                {
                        i++;
                        if(strcmpi(Player.name,player)==NULL || i>=8) { Close(fhpl); break; } 
                } 
                // test here
...
{       
        ...
        {                                
                ...
                } while(strcmpi(Player.name,player)!=NULL || i>=8); 
                ...
                sprintf(Player.name,"%s",player);
                sprintf(Player.laston,__DATE__); 
                
                Player.races=races;
                Player.wins = Player.wins+winflag;
                
                Player.cash=0; //...erstmal
         ...
         winflag=0; // reset winflag, sonst immer ++ !                            
...
void topten(void)
{
    ...
    char buf[256],oldname[32];
        ...
        PS("\033[2J");
        XIMFunction(8,0,"doors:sxcars/car_topten.ans");
        PS("[0;0H");
        PS("[6B");
        //oldname=="EMPTY";
        ...
        {                        
                ...
                while(Read(fhpl, &Player, sizeof(struct PlayerStruct)))
                {
                        ...
                        sprintf(buf,"\r[2B[5C%-12s[5C%5ld[8C%6ld[4C%12s",Player.name,Player.wins,Player.races,Player.laston);   
                        PS(buf);

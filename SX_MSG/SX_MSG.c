...
static char __version[] ="\0$VER: SX_MSG V1.3c bY [r]FoX at ("__DATE__")"; 
struct SXFuncStruct
{
      UWORD	id;
	 char	*string;
		      ...
		      if(strcmp(themsg.String,"SX")==0) sx=TRUE; else sx=FALSE;
	           ...
	           if(arg[0]=='s' || arg[0]=='S' || arg[1]=='s' || arg[1]=='S')
	           {
		           mailscan(0);
                     XIMFunction(2,0,0);
                     exit(0);             
                } else if(arg[0]=='i' || arg[0]=='I' || arg[1]=='i' || arg[1]=='I')
	           {
                     PS("[A");   
                     checkbases();
                     PS("[B");
...
void MessageLister(void)
{
        ...
        {                
                ...
                {                          
                        ...
                        if(strcmpi(oldbase,basnam)==0 || maxbase>=500) break; 
...
{        
        char buf[512],buffer[256],pathbuf[256],newansi[4],lrp_flag[8];
        static char oldbase[256]; 
        int baseindex=1;       
        BPTR fhbi,fh;
...
sprintf(buf,"%s\033[32m(\033[33m%3ld\033[32m) - \033[36m%-19s\033[32m[\033[31m%4s\033[32m/%s\033[32m]",newansi,baseindex,name,maxmsg,lrp_flag);
                        ...
                        if(strcmp(oldbase, name)==0) break;
                        ...
                        sprintf(oldbase,name);  // aktuelle msgbase in oldbase speichern
                        baseindex++; 
                               
                        if(baseindex>=256) break;  // !!!!!!!!!!!!!!!!!!! 56 ?
...
{                                                   // Messages in der richtigen MsgBase zu lesen
        ...
        char username[32],writer[32],reciver[32],fileadd[4];
        ...
        {  
                ...
                while(Read(fh, &msg, sizeof(struct MsgStruct))) 
                {
                        ...
                        if(msg.file[0]!=0) sprintf(fileadd,"+");
                        else sprintf(fileadd,"");
                        
                        msg.from[8]=0;
                        msg.to[8]=0;
                         
                        // Print the full String including all parms to the List on Screen
sprintf(buf,"[32m([35m%4ld[32m) [36m%10s [33m%-8s %-8s [31m%s[34m%s[35m%s[32m%s\r\n", msg.number, timestring, msg.from, msg.to, lrpflag, fileadd, reply, subject);
                        PS(buf);
...
{
        static char buf[512],buffer[256],pathbuf[256],oldbase[256],lrp_flag[8],newansi[8];
        static char confname[45],oldconfname[45],newbuf[256],username[32];                           
        ...
        sprintf(oldconfname,"EMPTY");                   // oldconfname - needed to stop reading the confs
        ...
        while(fhc)                                      // fh          - filehandle msgbases
 	     {                                               // fhc         - filehandle conferences
                ...
                //confnumba++;                            // ceck the users conferences access level here
					 //XIMFunction(1513,0, confnumba);
                XIMFunction(614, confnumba, 0);         // JH_CONFACCESS #614 - undocumented
                if(themsg.Data==NULL) end++;            // SX_CONFACCESS #1513
                ...
                if(fhbi==0) goto Ignore;
                else                   // double listing mit `q` - wenn 2 bases getyped dann return next line            
                {                                  
                        int q=0;
                
                        static char name[64],maxmsg[8];
                                                ...
                                                {                                                              
                                                        ...
                                                        {                                                                                                                        
                                                           ...
                                                           {                                                                                                                                
                                                                ...
                                                                XIMFunction(6,0," ");
                                                                if(themsg.String[0]==13 || themsg.String[0]=='y' || themsg.String[0]=='Y')
                                                                {                              
                                                                        sprintf(buf,"%u",baseindex);
...
PS("`-----------------------------------------------------------------------------'[32m"); 
        ...
        sprintf(buf," -[ SX_MSG V1.3 bY [r]FoX ]- ");
        PS("[2;49H");
        ...
        PS("[4;49H[1;33;44mwww.luebeck.netsurf.de/~clasen[0m[34m|");
